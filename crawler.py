...
class PttWebCrawler(object):
    ...
    def run_from_cli(self, cmdline=None):
    ...
    def parse_articles(self, start, end, board, timeout=3):
        articles = []
        for i in range(end - start + 1):
            index = start + i
            print('Processing index:', str(index))
            resp = requests.get(
                url=self.PTT_URL + f'/bbs/{board}/index{index}.html',
            ...
            for div in divs:
                try:
                    ...
                    article_json = self.parse(link, article_id, board)
    ...
    def parse_article(self, article_id, board, timeout=3):
        link = self.PTT_URL + f'/bbs/{board}/{article_id}.html'
        article_json = self.parse(link, article_id, board, timeout=timeout)
